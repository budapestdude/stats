...
const db = new sqlite3.Database('chess-stats.db', sqlite3.OPEN_READONLY);

console.log('Checking chess-stats.db (root) schema and sample data...\n');
...
db.all("SELECT sql FROM sqlite_master WHERE type='table'", (err, tables) => {
  ...
  // Check if there's a games table with moves/pgn
  db.get("SELECT COUNT(*) as count FROM games", (err, row) => {
    ...
    // Get a sample game to see all columns
